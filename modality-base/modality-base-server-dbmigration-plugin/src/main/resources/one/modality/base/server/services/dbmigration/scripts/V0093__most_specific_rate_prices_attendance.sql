-- V0093: the most specific applicable rate prices each attendance - event, then event type, then generic.
--
-- V0092 let the most specific rate win only among equally priced rates. The rule is now that a rate bound
-- to the booked event wins over one bound to its event type, which wins over a generic rate, whatever their
-- prices; price decides only among rates of the same specificity. A rate that does not apply to an attendance
-- (dates, withItem, withAccommodation, minDay...) is skipped as before, so a less specific rate still prices
-- the attendances a more specific one does not cover.
--
-- Consequences handled in the body:
-- - Candidates are ordered by specificity first (then unit price, share-owner per-room, rate id). The
--   no-attendance branch (fixed fees) takes the first matching rate, so it takes the most specific one; it
--   now also skips rates whose on/off dates exclude the line's creation, or a lapsed specific fee would win
--   over a valid generic one. When no fee rate matches, no rate supplies the line's min deposit and
--   non-refundable amounts (the last rate looked at used to).
-- - A rate applying only as a ceiling, for a stay shorter than its min_day, caps the price: it competes at
--   the lower of its own specificity and that of the most specific other applicable rate. It thus still
--   caps the rates of its own scope (an event's long-stay rate capping the event's daily rate), but never
--   overrides a less specific rate, so a short stay is not charged a specific long-stay cap instead of a
--   cheaper generic daily price. The rate is therefore chosen once all rates of the attendance are evaluated.
-- - A less specific fixed rate no longer counts the prices a more specific rate charged towards its own
--   ceiling: its window starts again the next time it applies, instead of making the remaining days free.
--
-- Known limits, unchanged: on a share-owner line the per_person multiplier comes from the rate applied to
-- the line's last attendance (V0039), and equally specific per_person and per-room rates are compared
...
-- next triggers compute_document_prices for them.
--
-- The client calculators (SiteItemBill in kbs3-react and in modality-fork) apply the same rule.
--
-- Body = scripts/compute_document_prices.sql in the aggregate repo.
      ...
      foreach rec in array recs loop
          ...
          if (not document.abandoned and not document_line.abandoned and not attendance is null) then
              ...
              foreach rate in array rec.rates loop
                  ...
                  if (not rate.per_day and kbs_overlaps(document_line.creation_date, document_line.creation_date, rate.on_date, rate.off_date)) then
