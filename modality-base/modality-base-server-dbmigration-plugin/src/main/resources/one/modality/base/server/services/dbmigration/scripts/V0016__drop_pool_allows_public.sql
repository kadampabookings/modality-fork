-- V0016: remove the legacy public pools and the allows_public column.
--
-- Public (online, non-reserved) beds have been represented by
-- document_line.pool_id NULL since the reserved-beds migration; the
-- "General public" / "Attendees" pools (the only allows_public=true rows) are
-- dead weight and were a source of confusion. They have ZERO references
-- (resource_configuration / document_line / event.default_pool_id all 0 —
-- verified on prod), so the FK constraints let them go, and KBS2 no longer
-- reads or writes them (commit 98e43271).
--
-- With those pools gone, requested_pool_id can never resolve to a public pool,
-- so V0008's allows_public guard in deferred_allocate_document_line() is dead
-- code. The function below is V0008's byte-for-byte MINUS that guard.
--
-- Order (one transaction): delete the pools (needs the column) -> replace the
-- trigger so it no longer reads allows_public -> drop the column.

-- 1. Delete the public-partition pools (the allows_public=true rows).
DELETE FROM pool WHERE allows_public;

-- 2. Replace the allocation trigger with V0008's body minus the allows_public guard.
...
ALTER TABLE pool DROP COLUMN IF EXISTS allows_public;
