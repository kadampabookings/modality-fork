-- V0092: among equally priced rates, the most specific one is applied.
--
-- BUG (event 1937, a Day Course): the T&Cs require a 100% deposit on day-course meals, so the
-- site's Breakfast/Lunch/Dinner rates were duplicated with event_type = Day Course and
-- min_deposit = 100. Bookings still got the 25% default deposit. The duplicates cost the same as
-- the generic rates, and compute_document_prices only replaces the chosen rate with a STRICTLY
-- cheaper one. Candidates were ordered by unit price alone, so among equal prices whichever row
-- the query returned first was kept - in practice the older generic rate - and its min deposit
-- and non-refundable percentages were applied.
--
-- FIX: equally priced candidates are now ordered most specific first - an event rate, then an
-- event-type rate, then a generic one - with rate id as the last key, so the choice no longer
-- depends on physical row order. The cheapest rate still wins. What can change is which of several
-- equally priced rates is applied, and therefore min_deposit / non_refundable.
--
-- Share-owner lines: the unit price used for ordering ignores per_person, but V0039 multiplies a
-- per_person rate by the number of sharers. Preferring a specific per_person rate over an equally
-- priced per-room one would therefore double a twin-room price, so on share-owner lines per-room
-- rates are ordered first, before specificity. The front end already compares per-person prices
-- multiplied by the sharers, so it reaches the same choice.
--
-- The no-attendance branch (fixed fees) takes the first matching rate in the same array, so it
-- follows the same order. Existing bookings are NOT recomputed by this migration: their stored
-- deposit changes only when something next triggers compute_document_prices for them.
--
-- The front end mirrors this tie-break in SiteItemBill (kbs3-react) so the booking form shows the
-- same deposit; the policy rate query returns rate.event and rate.eventType for that.
--
-- Body = V0039's compute_document_prices, with only the rate ORDER BY in the dlrs CTE changed.
...
BEGIN
      ...
      recs := array(
          ...
          coalesce(r.max_day, 1) end desc, case when (dls.dl).share_owner then coalesce(r.per_person, false) end, r.event_id is not null desc, r.event_type_id is not null desc, r.id) as rates
              from dls)
