-- V0008: harden deferred_allocate_document_line() against legacy PUBLIC pools,
-- and repair the lines they mismarked.
--
-- The V0004 engine treats ANY non-null requested pool (NEW.pool_id, or
-- event.default_pool_id on INSERT) as a reserved-bed request. That assumption
-- holds for the new model (pool = the REASON beds are withheld), but KBS2's
-- EditBookingOptionAction still exposes `pool` as an editable field, and staff
-- following the pre-migration workflow ("a booking in G8 consumes G8's
-- 'General public' pool") re-introduce PUBLIC pools on lines. The trigger then
-- marks those lines reserved=true with the public pool — wrong partition
-- counting and a nonsense reason label (observed: 25 lines on event 1887,
-- rooms 44/G8/G8A/G8B, user edits of 2026-07-07).
--
-- 1. The function below is V0004's byte-for-byte, plus ONE guard: a requested
--    pool with allows_public=true is treated as no pool at all.
-- 2. The repair pass clears public-pool markers (and the reserved flag they
--    wrongly implied) from document lines. It fires defer_allocate, whose
--    deferred re-allocation at COMMIT runs under the NEW function with
--    backend semantics (transaction parameters set below), keeping each
--    line's explicit resource_configuration.
--
-- Idempotent: the guard is CREATE OR REPLACE; the repair matches nothing on a
-- second run.
...
-- trying to allocate through automatic allocation rules (usually for meals dining areas)
            ...
            IF NOT FOUND THEN
                ...
                -- V0008: PUBLIC pools are legacy booking SOURCES (the pre-migration
                -- model), never reserved-bed REASONS — a public requested pool must
                -- not turn the line into a reserved request. KBS2's
                -- EditBookingOptionAction still lets staff set them (observed on
                -- event 1887, 2026-07-07: 25 lines came out reserved=true with
                -- pool='General public'), and a misconfigured event.default_pool_id
                -- would do the same.
                if requested_pool_id is not null
                   and exists(select 1 from pool p where p.id = requested_pool_id and p.allows_public) then
                    requested_pool_id := null;
                end if;
...
-- Backend transaction semantics for the deferred re-allocations the repair
-- triggers at COMMIT (same pattern as V0003's A0).
DO $$ BEGIN PERFORM public.set_transaction_parameters(true); END $$;

-- Repair: public pools are not partition markers — clear them and the
-- reserved flag they wrongly implied. (A5 did this at migration time; these
-- are the rows re-stamped since.)
UPDATE public.document_line dl
   SET pool_id  = NULL,
       reserved = false
  FROM public.pool p
 WHERE p.id = dl.pool_id
   AND p.allows_public;
